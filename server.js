...
app.get('/sitemap.xml', (req, res) => {
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        <url><loc>https://manofox.in/</loc><lastmod>${new Date().toISOString()}</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>
        <url><loc>https://manofox.in/admin</loc><changefreq>monthly</changefreq><priority>0.5</priority></url>
    </urlset>`);
});

app.get('/', async (req, res) => {
    let seo = await Seo.findOne({ pageName: 'home' });
    if (!seo) seo = { title: "Manofox", desc: "Digital Marketing", keywords: "seo" };
...
app.get('/admin', requireLogin, async (req, res) => {
    try {
        ...
        // 2. FETCH GOOGLE SEARCH DATA (NEW!)
        // This talks to Google's API to get clicks/impressions
        const googleSearchData = await getSearchData();

        // 3. Fake Bounce Rate/Time (since we don't have frontend scripts for this)
        ...
        res.render('admin', { 
            ...
            recentVisits, 
            googleSearchData // Sending GSC data to the frontend
        });
